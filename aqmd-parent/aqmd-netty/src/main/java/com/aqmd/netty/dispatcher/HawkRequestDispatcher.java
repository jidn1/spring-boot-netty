    ...
    private ResponsePacket doInvoke(RequestPacket request, ChannelHandlerContext ctx, HawkMethodHandler hawkMethodHandler, ResponsePacket response) {
        try {
            ...
            if (!(e instanceof InvalidProtocolBufferException) && !(e.getCause() instanceof InvalidProtocolBufferException)) {
                this.logger.error("指令{}#{}未知错误, {}", new Object[]{request.getCmd(), request.getVersion(), Throwables.getStackTraceAsString(e), e});
                response.setBody(CommonResult.newBuilder().setResultCode(NettyResponseCode.UNKNOW_ERROR.getResponseCode()).setResultMsg(e.getMessage()).build().toByteArray());
            } else {
                this.logger.error("指令{}#{}数据包格式错误，{}", new Object[]{request.getCmd(), request.getVersion(), Throwables.getStackTraceAsString(e)});
                response.setBody(CommonResult.newBuilder().setResultCode(NettyResponseCode.BODY_FORMAT_ERROR.getResponseCode()).setResultMsg(e.getMessage()).build().toByteArray());
            }
        }
